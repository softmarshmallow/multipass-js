...
class Multipass {
  ...
  withCustomerData(customer: IMultipassCustomerData | unknown): Multipass {
    ...
    this.customer = customer as IMultipassCustomerData;
    return this;
  }
  ...
  url(): string {
    if (!this.customer) {
      throw new Error(
        "cannot generate url. you did not provided any customer information"
      );
    }
    if (!this.domain) {
      throw new Error(
        "cannot generate url. you did not provided any domain information"
      );
    }
    return `https://${this.domain}/account/login/multipass/${this.encode(
      this.customer
    )}`;
  }
  ...
  private encode(obj: IMultipassCustomerData): string | undefined {
    ...
    let token = Buffer.concat([cipherText, this.sign(cipherText)]).toString(
      "base64"
    );
    token = token
